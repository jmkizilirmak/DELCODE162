...
% variants. NaN exclusion accoring to SVM variant with the most features. 4
% classes (CN, SCD, MCI, AD) are included, but SCD and MCI are collapsed to
% form one single class "AD risk".
...
%% specify directories
SVM_dir = '\\dzne-go-csv\Schott-Lab\commons\162_DELCODE_Personality\SVM_PerAF\SVMs_same_subjs_riskclass_noADrel\'; % output directory
...
data_imgs = data_imgs(empty==0 & x~=4);
X         = X(empty==0 & x~=4,:);
c         = x(empty==0 & x~=4);
...
Xselect{7} = deal(X(:,1:13));               % confounding only (base model); exclude neuroimaging !!!
